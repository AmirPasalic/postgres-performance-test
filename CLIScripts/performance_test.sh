...
function help {
    echo ""
    echo "NAME"
    echo "$TAB ppt_performance_test"
    echo ""
    echo "DESCRIPTION"
    echo "$TAB ppt_performance_test help command page"
    ...
    echo ""
    echo "$TAB ppt_performance_test help command will run the test queries on the CarReservationsDb."
    echo "$TAB Each query run on CarReservationsDb will run on a Standard SQL based schema and on JSONB based schema."
    echo "$TAB By default the command: ppt_performance_test will run the queries on the database only once and"
    ...
    echo "$DOUBLE_TAB Example would be ppt_performance_test -c 3"
    echo ""
    ...
    echo "$DOUBLE_TAB Example would be ppt_performance_test -wi"
